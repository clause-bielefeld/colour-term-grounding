...
# import arrays
bottomup_arrays = np.load(input_dir+'histograms_bgr_resampled.npz', allow_pickle=True)
...
print ('Train-IDs identical:', np.array_equal(train_bu_x[:,0], train_td_x[:,0]))
print ('Dev-IDs identical:', np.array_equal(dev_bu_x[:,0], dev_td_x[:,0]))
print ('Test-IDs identical', np.array_equal(test_bu_x[:,0], test_td_x[:,0]))
print ('Add-IDs identical', np.array_equal(add_bu_x[:,0], add_td_x[:,0]))

# prepare pretrained embeddings

# get object types (as int)
x_type = np.concatenate([train_td_x[:,1:],dev_td_x[:,1:],test_td_x[:,1:],add_td_x[:,1:]])
x_type = np.argmax(x_type, axis=1)

obj_ids = np.concatenate([train_ids, dev_ids, test_ids, add_ids])
# match type-numbers with object names
...
td_embedding = Embedding(vocab_size, 100, weights=[embedding_matrix], input_length=1, trainable=False, name='td_embedding')(td_inputs)

flatten = Flatten(name='td_flatten')(td_embedding)

post_embedding = Dense(24, activation='relu')(flatten)
post_embedding = Dropout(0.2)(post_embedding)

conc = concatenate([bu_inputs, post_embedding],axis=1, name='bu_td_concat')
...
#plot_model(model, to_file=data_dir+'images/earlyfusion_plot.png', show_shapes=True, show_layer_names=False)
